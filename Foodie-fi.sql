...
select count(distinct customer_id) from lead_table
...
-----final cohort table
select c.month_first_join,
        t.total_trial,
        a.total_user_after_trial,
        c.total_user as total_user_lifetime,
        c.month_lifetime
from month_trial_table t 
left join month_after_trial_table a
on t.month_num = a.month_num
left join cohort_customer_lifetime c
on t.month_num = c.month_first_join
order by t.month_num asc, c.total_user desc, c.month_lifetime asc
