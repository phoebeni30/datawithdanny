...
--##############################################################
--#00 /extras answer/ This is code for category review on 3 films that customer has watched most in their historical films by categories
DROP TABLE IF EXISTS category_film_history;
with cte_film_ranking as (
select customer_id, 
        category_name, 
        title, 
        count(*) as frequency_film_count,
        max(rental_date) as lastest_rental_date
from completed_join_dataset
group by customer_id, category_name, title)
,
cte_top_category_ranking as (
select cte_film_ranking.customer_id, 
        cte_film_ranking.category_name, 
        cte_film_ranking.title,
        cte_film_ranking.frequency_film_count,
        ROW_NUMBER() over(partition by cte_film_ranking.customer_id, cte_film_ranking.category_name 
                            order by cte_film_ranking.frequency_film_count desc)
        as film_ranking
from cte_film_ranking 
left join top_2_category 
on cte_film_ranking.customer_id = top_2_category.customer_id
where cte_film_ranking.category_name = top_2_category.category_name 
        and top_2_category.category_ranking in (1,2)
)
select *
into category_film_history --create temp table category_film_history
from cte_top_category_ranking
where film_ranking in (1,2,3)
order by customer_id asc
-------Check temp table category_film_history
select * from category_film_history
